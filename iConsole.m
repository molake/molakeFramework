...
@property (nonatomic, assign) BOOL sendingNSLog;
...
//        [_consoleView setContentOffset: CGPointMake(0, _consoleView.contentSize.height) animated:NO];
...
- (void)infoAction
{
    ...
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:nil
                                         ...
                                         destructiveButtonTitle:@"Clear Log"
                                              otherButtonTitles:@"Send All", @"Close Console", nil];
...
- (void) sendAndDeleteNSLog: (BOOL) isAll{
    if([_log count] == 0){return;}
    if(_logSubmissionURL != nil){
        if(_sendingNSLog == YES){return;}
        // 送信するリクエストを生成する。
        NSMutableURLRequest *request = [NSMutableURLRequest new];
        [request setURL: [NSURL URLWithString: _logSubmissionURL]];
        [request setHTTPMethod: @"POST"];
        [request setTimeoutInterval: 30];
        [request setValue: @"text/plain" forHTTPHeaderField: @"Content-Type"];
        [request setHTTPBody: [[@"yarukeylog\n" stringByAppendingString: [(isAll? _log: [_log subarrayWithRange: NSMakeRange(0, _autoSendLogItems)]) componentsJoinedByString: @"\n"]] dataUsingEncoding: NSUTF8StringEncoding]];
        // リクエストを送信する。
        _sendingNSLog = YES;
        // 第３引数のブロックに実行結果が渡される。
        [NSURLConnection sendAsynchronousRequest: request queue: [NSOperationQueue mainQueue] completionHandler: ^(NSURLResponse *response, NSData *data, NSError *error){
            if(error == nil){
                ...
                if(httpStatusCode == 200){
                    [self deleteNSLog: isAll];
                    self.sendingNSLog = NO;
                    if([_log count] > _maxLogItems){
                        [self sendAndDeleteNSLog: isAll];
                    }
                }else{
                    NSLog(@"error %ld: %@", httpStatusCode, [[NSString alloc] initWithData: data encoding: NSUTF8StringEncoding]);
                    self.sendingNSLog = NO;
                }
            }else{
                if(error.code == -1009){
                    if(isAll){
                        NSLog(@"cannot send log: offline");
                    }else if([_log count] > _maxLogItems){
                        NSLog(@"over maxLogItmes. send log online or clear log.");
                        [iConsole show];
                    }
                }else{
                    NSLog(@"cannot send log - error: %@", error);
                }
                self.sendingNSLog = NO;
            }
        }];
...
- (void) deleteNSLog: (BOOL) isAll{
    if(isAll){
        self.log = [NSMutableArray array];
    }else{
        [self.log removeObjectsInRange: NSMakeRange(0, self.autoSendLogItems)];
    }
    if(self.view.superview){
        [self setConsoleText];
    }
...
//        self.logSubmissionEmail = nil;
        ...
        self.logSubmissionURL = nil;
        self.autoSendLogItems = 500;

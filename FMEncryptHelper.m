...
+ (void)initialize
{
    ...
    encryptKey_ = settings[@"DatabaseEncryptedKey"];
}
...
+ (BOOL)encryptDatabase:(NSString *)sourcePath targetPath:(NSString *)targetPath
{
    ...
    if (sqlite3_open([sourcePath UTF8String], &unencrypted_DB) == SQLITE_OK) {
        //sqlite3_exec(unencrypted_DB, [[NSString stringWithFormat:@"PRAGMA cipher_default_use_hmac = off;"] UTF8String], NULL, NULL, NULL);
